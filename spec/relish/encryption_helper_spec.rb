...
describe Relish::EncryptionHelper do
  ...
  let(:secret_two)     { 'b' * 32    }
  let(:data)           { 'test-data' }
  let(:encrypt_helper) { Relish::EncryptionHelper.new('static_secret1', [secret_one]) }
  ...
  describe '#decrypt' do
    it 'raises RelishDecryptionFailed on incorrect decryption secret' do
      ...
      decrypt_helper = Relish::EncryptionHelper.new('static_secret1', [secret_two])
    ...
    it 'decrypts data encrypted with a previous secret' do
      token = encrypt_helper.encrypt(data)
      decrypt_helper = Relish::EncryptionHelper.new('static_secret1', [secret_two, secret_one])
    ...
    it 'decrypts data encrypted with an array as secret' do
      token = encrypt_helper_array.encrypt(data)
      assert_equal data, encrypt_helper.decrypt(token)
    end

    it 'decrypts data encrypted with a string as secret' do
      token = encrypt_helper.encrypt(data)
      assert_equal data, encrypt_helper_array.decrypt(token)
    end

    it 'decrypts data encrypted with a previous array of secrets' do
      token = encrypt_helper_array.encrypt(data)
      assert_equal data, encrypt_helper_array.decrypt(token)
    end
  end
  ...
  describe '#encrypt' do
    ...
    it 'encrypts with array of secrets' do
      token = encrypt_helper_array.encrypt(data)
      assert_equal data, encrypt_helper_array.decrypt(token)
    end
  end
